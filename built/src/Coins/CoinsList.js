 ...
 */
const CoinsList = observer(({ navigation }) => {
    useEffect(() => {
    ...
    const renderItem = ({ item, index }) => {
        return (<CoinRow onPress={showDetails(index)} coinName={item.coinName} imageUrl={item.imageUrl}/>);
    };
    return (<SafeAreaView edges={['bottom']} style={{ flex: 1, backgroundColor: 'white' }}>
      {store.coinsStore.isCoinListLoading && store.coinsStore.page === 0 ? (<ActivityIndicator size="large" style={styles.activityIndicatorFullScreen}/>) : (<FlatList data={store.coinsStore.coinsList} renderItem={renderItem} keyExtractor={({ id }, index) => id} ListFooterComponent={<ActivityIndicator style={styles.activityIndicatorBottom}/>} onEndReachedThreshold={0} getItemLayout={(data, index) => ({
                length: 75,
                offset: 75 * index,
                index,
            })} maxToRenderPerBatch={20} onEndReached={() => {
                store.coinsStore.fetchCoinsList();
